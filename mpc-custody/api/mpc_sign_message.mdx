...
<Note>This api is only used for MPC Web3 wallets</Note>
...
<ParamField body="extra_parameters" type="Json" required>extra_parameters should include field message in EIP-191 signature, which is Base64 encode of the original message(e.g., ```"{\"message\": \"YWFhYQ==\"}"```), and should contain filed structured_data in EIP-712 signature, which is Json of EIP-712 structured data, please refer to https://eips.ethereum.org/EIPS/eip-712 for EIP-712 info</ParamField>
