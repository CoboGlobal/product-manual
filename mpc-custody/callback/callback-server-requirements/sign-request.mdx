...
<ParamField body="api_request_id" type="String" required><p>Request ID of the withdrawal request;</p><p>If the request is initialized via API, the value should be the same as request_id;</p><p>If the request is initialized via Cobo Custody Web, Cobo will automatically generate a Request ID.</p></ParamField>
