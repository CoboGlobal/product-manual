...
**Note**: In the unlikely event of Cobo Portal discontinuing its services, you can reconstruct the root extended private keys corresponding to the vault by utilizing an active Recovery Group. All private keys linked to the wallet addresses within this vault can be derived from these root extended private keys. Unlike restoration, recovery relies on the support from another key share group.
...
- Obtain the recovery kit from the open-source repository: [cobo-mpc-recovery-kits](https://github.com/CoboGlobal/cobo-mpc-recovery-kits). Prior to compilation of cobo-mpc-recovery-tool, familiarize yourself with the instructions in the README.md file. Alternatively, download the latest version of cobo-mpc-recovery-tool [here](https://github.com/CoboGlobal/cobo-mpc-recovery-kits/releases).
...
- Save the wallet addresses associated with each vault using one of the following methods:
   - Retrieve AddressDetail via the generate_addresses API during key generation. The HD path corresponding to each address will be included in the response.
   - Retrieve AddressDetail via the list_addresses API regularly. The HD path corresponding to each address will be included in the response.
...
├── db
│ └── secrets.db
├── recovery (default folder where the exported private key shares will be stored)
└── tss-node.sh
```
...
8. The private key shares will be automatically exported as JSON files. Please verify the accuracy of the root extended public keys and group ID in the exported files.

## Step 2: Reconstruct all private key shares and derive the private keys for all wallet addresses associated with the vault (applicable to both mobile co-signer and API co-signer).
...
```
3. Execute the following commands using the actual group IDs and private key share files.
```
...
--group-id <GROUP_ID>
```
4. Wait for the MPC recovery tool to automatically complete the verification process.
...
5. Place the address.csv file exported during pre-preparations under the recovery folder.
```
├── cobo-mpc-recovery-tool
└── recovery
├── address.csv
...
6. Execute the following commands using the actual group ID and private key share files.
