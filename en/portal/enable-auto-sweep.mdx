...
Once you enable auto token sweeping, Cobo automatically generates a dedicated sweep-to address for each enabled chain in the wallet. These addresses are system-assigned and cannot be manually specified or modified. When any deposit address receives tokens, the system will automatically and periodically sweep these tokens to the corresponding sweep-to address. The wallet addresses are displayed under two tabs: the **Deposit** tab shows previously created deposit addresses, while the **Sweep-To** tab shows the sweep addresses.
...
By default, deposit addresses cannot initiate withdrawals, contract calls, staking, OTC trades, or smart contract delegations. To manually enable these capabilities for deposit addresses, follow these steps in [Enable deposit address transactions](/en/portal/deposit-address-transaction).
