...
In Cobo Portal, enabling auto token sweeping is a one-time action. Pausing or resuming sweeping is a separate setting available only for Custodial Wallets (Web3 Wallets): it temporarily controls future automatic triggers without reversing enablement and does not apply to MPC Wallets.
...
For Custodial Wallets (Web3 Wallets), you can pause sweeping when encountering unexpected situations such as network congestion, fee fluctuations, risk control checks, or temporary business adjustments. After conditions return to normal, resume sweeping to continue according to the established strategy. This setting is not available for MPC Wallets. For more details, refer to [Pause/Resume sweeping](/en/portal/auto-sweep-pause-resume).
