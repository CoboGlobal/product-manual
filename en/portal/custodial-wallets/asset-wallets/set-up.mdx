...
<Note>Asset Wallets use the same address across all supported EVM chains. You only need to create it once. Existing addresses on one EVM chain will automatically work on all other supported EVM chains.
...
- Cobo Portal: Displays addresses from all supported EVM chains, so the number of addresses may be larger than the API results.</Note>
