...
### **Introduction to off-the-shelf server**

An off-the-shelf server is any server that satisfies the minimum configuration prerequisites for the TSS Node, such as Elastic Compute or a physical server managed by the client. While an off-the-shelf server can host the TSS Node, it lacks the distinctive security features inherent in an SGX-ready server.
