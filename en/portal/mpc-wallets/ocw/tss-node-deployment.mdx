...
The TSS Node package is available in two versions based on deployment methods:

- SGX security enhanced version: Suited for SGX-ready servers, the package is named cobo-tss-node-linux-sgx-\<VERSION\>.tgz.
- Off-the-shelf version: Designed for off-the-shelf servers or Apple MacBooks, the package is named cobo-tss-node-generic-\<VERSION\>.tgz.

To obtain the appropriate TSS Node package for your deployment method, please reach out to Cobo's customer support at [support@cobo.com]. The subsequent example employs the off-the-shelf version of the TSS Node package.

```
cobo-tss-node-generic-<VERSION>.tgz (TSS Node file)

packagecobo-tss-node-generic-<VERSION>.tgz.sha256 (hash file）
```
...
The TSS Node command lines remain consistent across all three operating environments, namely the SGX-ready server, off-the-shelf server, and Apple MacBook. However, the execution of command lines may vary slightly. For both SGX-ready servers and off-the-shelf servers, sudo access is essential for deploying and running TSS Node. In other words, ensure that sudo commands are inserted.
...
Unless explicitly stated otherwise, all subsequent commands in this user guide will use the off-the-shelf server as an example.
...
<Note>Only the client with Vault Admin rights is authorized to add the TSS Node ID to Cobo Portal.</Note>

1. Log in to Cobo Portal.
2. Click **Wallets** in the main menu on the left hand side.
3. Select **MPC** and switch to **Organization-Controlled Wallet.**
4. Click **Create Main Group.**
5. Ensure that the TSS Node managed by Cobo is online to jointly generate the root extended public key.
6. Select **API Co-Signer.**
7. Enter a name for this API Co-Signer.
8. Enter the TSS Node ID of this API Co-Signer and click **Confirm**.
9. Ensure that the root extended public key displayed on Cobo Portal matches the one shown in the TSS Node log. This key can be used to generate all wallet addresses under this MPC Organization-Controlled Wallet using BIP 32.

![img](https://lh7-us.googleusercontent.com/goJ5sSXRCe4YIlvQE4Ln0MG2nHUs6vfSUXbCytDLSMTBgQjl-dXLJBfpHE4VuwJuEx4O40EmeqQcMqxC2WsdFi0yo-ygfiHIbfQSEDkhXUnHsFeUrR5ISu68GgO0S90hA5Z46k_LMHBSJ7MtnLxfZBo)

 

### **Private key share management** 

The successfully generated private key shares will be encrypted and stored locally in the database file of the TSS Node package. The default path is db/secrets.db. It is highly recommended to create backups of the database file and the password used for encryption during the initialization of the TSS Node. The backup files should be stored on separate devices for enhanced security. For more information, please refer to [Back up key share groups](/en/portal/mpc-wallets/ocw/back-up-key-share-groups).