...
In an Organization-Controlled Wallet, the management and control of the wallet lie with the organization itself. This setup allows organizations to securely safeguard and maintain complete control of their own funds or their end users’ assets.

An Organization-Controlled Wallet utilizes a 2-2 signature scheme, with one key share held by the organization and the other key share stored within Cobo's multi-cloud infrastructure.
...
To integrate non-custodial MPC Wallets into your applications, a 2-2 signature can be implemented, where the end user holds a key share and the other key share is stored within Cobo's multi-cloud infrastructure, while the organization holds none.
