...
You can effortlessly create and edit transaction policies, and automate how each transaction will be handled by setting up an approval action. 
<Note>Transaction policies are managed by the backend system of Cobo Portal and apply to all wallet types.</Note>

It is recommended to strictly configure transaction policies, especially for large withdrawal scenarios. Particular attention should be given to token transfer policies to avoid high-risk situations, such as:
...
To enhance security and reduce the risk of account compromise, it is strongly recommended that all Admins enable at least two authentication methods, with Portal Mobile as one of them.

Recommended combinations include:
- Portal Mobile and Google Authenticator (GA)
- Portal Mobile and security key

<Note>Install GA on a different device than Portal Mobile to prevent loss of access if one device is lost.</Note>
