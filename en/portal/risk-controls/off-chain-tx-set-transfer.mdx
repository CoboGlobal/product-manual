...
### Quota calculation <a id="quota-calculation"></a>

When you set a spending limit over a time window using **Frequency**, Cobo Portal tracks a running total of the qualifying transaction amounts within that window. The policy is triggered when a new transaction would cause the running total to exceed the limit you set. This running total is calculated by Cobo's risk-control engine.
