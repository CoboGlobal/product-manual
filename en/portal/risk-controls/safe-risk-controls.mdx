...
<Info>After the policy is configured, Cobo Portal automatically starts monitoring the Safe Transaction Service to look for pending transactions from the corresponding Safe{Wallet}.</Info>
...
As mentioned previously, after the message signing policy is configured, Cobo Portal automatically monitors the Safe Transaction Service. When a pending transaction is found for the corresponding Safe{Wallet}, Cobo Portal initiates a signature request and starts the risk control process. Approvers will receive a notification in their [Cobo Guard App](/en/guard/introduction) to review the request. The Signer address will only sign the message after the required number of approvers (set in the approval quorum) have approved it. If any approver rejects the request, the message will not be signed.
...
- Marks high-risk factors with red labels that show risk details, for example:
   - For the **To** field: 
      - Shows **Never Traded Before** when the Safe{Wallet} interacts with a target contract or receiving address for the first time
      - Shows **New Contract** for contracts deployed less than 30 days (720 hours) ago
   - For the **Operation** field:
      - Shows **High-Risk Operation** for operations such as Delegate Calls

   <img src="/en/images/risk-controls/safe-signer-cobo-guard.png" className="screenshot_full_screen"/>

   For high-risk fields, click to review the details and confirm you understand the risk before you approve the message.
