...
7. Under **If**, click **+ Add Condition** and select **Safe&#123;Wallet&#125; Message Signing**. Map your Cobo Portal addresses (selected in **Applicable Wallets**) to the Safe&#123;Wallet&#125; addresses that they will sign for. The policy will be triggered when these Safe&#123;Wallet&#125; addresses request signatures.
   ...
   Toggle on **Custom Conditions** for enhanced security controls.
      - **Custom Delegate Call**: The policy will be triggered only if a delagate call meets the specified criteria. 
      - **Custom Contract Call**: The policy will be triggered only if a contract call meets the specified criteria.

   You can select multiple custom conditions (for example, both **Custom Delegate Call** and **Custom Contract Call**), and they use **OR** relationship - policy triggers if any condition is met.

   For each custom condition, you can select multiple contract address selections and one of the three levels of granularity:
      - **Contract-level**: You can specify the smart contract addresses for the call. The policy will be triggered when the **Target Contracts** of the contract call match one of the specified addresses.
      - **Method-level**: You can specify the smart contract address and methods for the call. The policy will be triggered if both the **Target Contract** and one of the **Methods** match the specified criteria.
