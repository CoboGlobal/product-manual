...
3. Click **Create Wallet**. In the dialog box:
   ...
   - If you need to automatically consolidate funds from multiple addresses, enable [auto token sweeping](/en/portal/mpc-wallets/ocw/auto-sweep-overview). If you're unsure, you can leave it disabled for now and enable it later if needed.
4. Click **Create**.
...
5. One of your co-signers will be automatically selected to sign the transaction based on their online status and other criteria. Ensure that at least one co-signer is ready to complete the signing process.
...
* [Enable auto token sweeping](/en/portal/mpc-wallets/ocw/auto-sweep-overview): Automatically consolidate funds from multiple addresses into a single designated address.
