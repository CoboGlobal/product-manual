...
To view the chains currently enabled for a specific wallet type, click the **plan badge** to the right of the organization name in the top navigation bar > the wallet type card you want to check (for example, **MPC Wallets**) > **Supported Chains** > **Manage**. The **Chain Management** panel opens, listing the chains currently enabled for the selected wallet type.
