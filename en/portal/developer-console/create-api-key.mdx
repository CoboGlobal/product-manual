...
<Note>Role permissions and Wallet Scope are separate controls. The role you select grants API operation permissions at the resource and operation level, while Wallet Scope limits which wallets those permissions can operate on. For wallet-targeted Custody and WaaS actions, both the role permission and Wallet Scope must allow the action. For the full permission matrix, see [Roles and permissions](/en/portal/risk-controls/roles-and-permissions).</Note>
...
8. Select user roles and wallet scope from the corresponding dropdown lists.
    ...
    Use **Shared scope** to apply one wallet scope to all selected roles. Click **For Each Role** to use **Per-role scope**, where each selected role can have its own wallet scope.
...
9. Select the callback endpoint this API key uses, or register a new one. The selected callback endpoint will receive messages triggered by the API key. For more information, see [Register a callback endpoint](/en/portal/developer-console/callbacks-register).
    ...
    <Warning>If no Callback Endpoint is configured for this API key, callback-based withdrawal double-check is skipped and withdrawal requests are approved by default.</Warning>
