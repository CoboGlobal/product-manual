...
<Note>Prerequisite: Before making a transfer from Exchange Wallets, please make sure that you have successfully [linked](/en/portal/exchange-wallets/set-up) to the designated exchanges, set up an [off-chain transaction policy](/en/portal/risk-controls/off-chain-tx-set-transfer) (recommended), and [deposited](/en/portal/exchange-wallets/transfers) funds into the Exchange Wallets.</Note>

To make a transfer from Exchange Wallets, log in to [Cobo Portal](https://portal.cobo.com/login) and then click <img src="/images/transfers/transfer-button.svg" className="icon"></img> in the main menu on the left hand side.
...
2. In the drop-down menu under ‘From’, select an exchange under **Exchange Wallets**.
3. To find your exchange account, select it from the drop-down menu or search by the wallet name. All exchange accounts linked through Cobo Portal and WaaS 2.0 API will be displayed.
...
Note: For the list of exchanges supported by Cobo Portal, please refer to [Supported exchanges](/en/portal/exchange-wallets/supported-exchanges).
