...
Under the EIP-1559 model, the estimated transaction fee is: (base fee + priority fee) * gas limit
...
Under the Legacy model, the estimated transaction fee is: gas price * gas limit
...
On UTXO-based chains, the estimated transaction fee is: fee rate * transaction size
...
On Solana, you can use the native model or choose the Legacy model. Under the native model, the estimated transaction fee is: base fee + (CU price * CU limit)
...
On Filecoin, you can use the native model or choose the Legacy model. Under the native model, the estimated transaction fee is: min(gas fee cap, base fee + gas premium) * gas limit
