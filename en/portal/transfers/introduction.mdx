...
The Transfer module is meticulously designed to streamline the movement of assets within, into, and out of the wallet types available on Cobo Portal. These encompass Custodial Wallets, MPC Wallets, and Exchange Wallets. Currently, Cobo Portal supports the following transfer scenarios:


1. Transfers from Custodial Wallets (Asset Wallets), MPC Wallets (Organization-Controlled Wallets), or Exchange Wallets to external addresses.
2. Transfers between Custodial Wallets (Asset Wallets), MPC Wallets (Organization-Controlled Wallets), and Exchange Wallets.
...
For transfers from MPC Wallets and Custodial Wallets (Web3 Wallets), Payment organizations use [Fee Station](/en/portal/fee-station/introduction) for eligible gas-fee handling. If Fee Station does not cover the operation, maintain the required native gas-token balance.
