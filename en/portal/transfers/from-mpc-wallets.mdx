...
3. For **Wallet Type**, select **MPC Wallets (Org-Controlled Wallets)**. For the wallet address, select from the drop-down menu or search by address, vault name, wallet name, or label. MPC Wallets (Organization-Controlled Wallets) created via both Cobo Portal and WaaS 2.0 API will be listed.
