...
- It's recommended that you set up an [transaction policy](/en/portal/risk-controls/off-chain-tx-set-transfer) to ensure transaction security.
- You have [deposited funds](/en/portal/mpc-wallets/ocw/transfers) into the Organization-Controlled Wallet.
- You have enough native token in your wallet to pay for the gas fee (on-chain transaction fee). For eligible gas-fee handling, Payment organizations use [Fee Station](/en/portal/fee-station/introduction). If Fee Station does not cover the operation, maintain the required native gas-token balance.
...
2. **Double confirmation**:
   - For Cobo Portal initiated withdrawals: Confirm the withdrawal on [Portal Mobile](/en/portal-mobile/introduction).
...
3. **[Risk control](/en/portal/risk-controls/tx-policies-overview)**: If configured, transaction policies in Cobo Portal assess the withdrawal request. When manual review is required, approvers must approve the transaction on Portal Mobile.
