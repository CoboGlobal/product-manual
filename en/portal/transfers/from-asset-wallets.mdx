...
2. Select a token type for the transfer. To enable a new token type, navigate to your Asset Wallet page and click **Add Tokens**.
3. In the drop-down menu under **From**, for **Wallet Type**, select **Asset Wallets**. For the wallet address, select from the drop-down menu or search by wallet name, address, or label. Asset Wallets created via both Cobo Portal and WaaS 2.0 API will be listed.
