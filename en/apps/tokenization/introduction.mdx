...
Support for more chains like Solana is coming soon.
...
- **Token access**: By default, all addresses not on the blocklist can transact freely. To enhance control, you can enable **Restricted Access** to activate an allowlist, limiting all token circulation exclusively to addresses that are on your allowlist AND not on the blocklist.
...
- **Write functions (executing actions)**
    ...
    - `accessListAdd`: Add one or more addresses to the token access list
    - `accessListRemove`: Remove one or more addresses from the token access list
