...
4. **Risk control and segregation of duties**: Internal approval workflows for token management operations, ensuring every action requires administrator authorization to reduce risks of misuse or error  
...
#### 2.2 Create tokens with Cobo’s Tokenization App  
a. Define core token parameters (name, symbol, etc.) and set operational permissions.  
b. Initiate token creation using an address under the Deployment Vault and complete contract deployment.  
c. After deployment, review and configure steps 3, 4, and 5 to ensure security and compliance requirements are met.  

#### 2.3 Token contract permissions  
**a. Definition**  
Contract-level “signing address configuration” granting addresses specific operational permissions.  
...
#### 2.4 Token transfer restrictions  
**a. Definition**  
Smart-contract-level controls on token ownership and transfers, ensuring only eligible wallets may hold and transfer tokens.  
...
- High-risk operations: require majority administrator approval  
- Routine operations: require approval from at least one administrator  
