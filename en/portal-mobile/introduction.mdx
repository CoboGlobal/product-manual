...
<Note>Currently, Cobo Portal Mobile is primarily intended for newly registered users. Accounts that have already been bound to Cobo Guard cannot yet complete device binding in Portal Mobile, and will see a corresponding message after logging in. For details, see [Migrate from Cobo Guard](/en/portal-mobile/migrate-from-guard).</Note>
...
- If you are a **new user or a new organization**, we recommend using Cobo Portal Mobile directly.
