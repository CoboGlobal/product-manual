...
<Warning>
**Important availability notice — Cobo Portal Mobile is currently for ELIGIBLE NEW USERS ONLY.**

- **Who can use it:** Only **new users and organizations provisioned by Cobo Sales** that have **never been bound to Cobo Guard**.
- **Who cannot use it (yet):** Any **existing user whose account is already bound to Cobo Guard**. These accounts **cannot bind a device, generate key shares, migrate, or otherwise use Portal Mobile** at this stage.
...
Portal Mobile is both the security hub of your account and your mobile entry point for viewing assets, initiating transactions, and approving operations anytime, anywhere. Current availability depends on the access provisioned for your account by Cobo Sales.
...
Portal Mobile supports Google and email sign-in on both platforms, plus Apple sign-in on iOS. Provisioned email accounts can sign in with a password or a one-time verification code. Portal Mobile does not provide registration for new accounts or organizations; contact Cobo Sales to request access. For details, see [Sign in to Cobo Portal Mobile](/en/portal-mobile/register-and-sign-in).

## Current app capabilities

The current Portal Mobile app also includes the following capabilities beyond the original onboarding and wallet flows:

- **Login & Security**: Link Google or Apple, set or change an email password, manage passkeys, and review linked login methods.
- **Address search**: Search wallet addresses and memo values from the address view, with matching text highlighted.
- **Token and chain management**: Open token management from a wallet to review and manage supported token/chain entries.
- **Account deletion**: Submit an account deletion request from **Me** after reviewing the consequences and completing MFA verification.
- **Organization selection**: Select an organization that Cobo Sales has provisioned for the account.
- **AI customization**: Complete the in-app AI preference questionnaire and continue to the Cobo Portal assistant.
- **Guard migration recovery**: The app contains guided recovery and re-backup screens for the supported Guard-to-Portal migration flow. Availability remains subject to the account eligibility rules in [Migrate from Cobo Guard](/en/portal-mobile/migrate-from-guard).

These capabilities may vary by account, platform, environment, and rollout status. The app's current UI and eligibility checks are authoritative.
...
<Warning>**Current limitation — eligible new users only, no migration from Cobo Guard yet.** Cobo Portal Mobile is currently available only to **new users and organizations provisioned by Cobo Sales that have never been bound to Cobo Guard**. Accounts already bound to Cobo Guard cannot complete device binding, generate key shares, or migrate in Portal Mobile. For details, see [Migrate from Cobo Guard](/en/portal-mobile/migrate-from-guard).</Warning>
...
- If you are a **new user or organization provisioned by Cobo Sales** that has never been bound to Cobo Guard, you can use Cobo Portal Mobile directly.
