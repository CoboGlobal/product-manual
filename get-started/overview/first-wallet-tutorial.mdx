...
3. How to retrieve transcation information.
...
1. You have pulled your preferred [SDK](/get-started/sdks/waas/python) for development.
2. You have completed [key pair generation](/get-started/sdks/waas/python#generate-key-pair) for API Authentication.
...
<Card title="Use the correct Client and Environment" icon="lightbulb" iconType="duotone" color="#ca8b04">
  ...
  `DEVELOP_ENV` is develop environment, `PROD_ENV` is for production environment.
</Card>
...
mpc_client.get_wallet_supported_coins()
```
```java Java
...
<CodeGroup>
```python Python
client.new_deposit_address("GETH")
...
<CodeGroup>
```python Python
client.get_transactions_by_time(side="deposit", limit="1")
mpc_client.list_transactions(transaction_type=1000, order_by="created_time", order="DESC", limit=1)
```
```java Java
client.getTransactionsByTime(null, Side.Deposit, null, 0, 0, 1, null);
mpcClient.listTransactions(null, null, null, "created_time", "DESC", 1000, null, null, null, 1);
```
</CodeGroup><br />
<Accordion title="View Response">
```json
...
<CodeGroup>
```python Python
client.get_coin_info("GETH")
mpc_client.estimate_fee("GETH",1,"To_Address")
```
```java Java
client.getCoinInfo("GETH");
mpcClient.estimateFee("GETH", withdraw_amount, fromAddr, null,
...
client.withdraw("GETH","To_addres","10000000000000000",request_id)
mpc_client.create_transaction("GETH",request_id,"From_address","To_Address","10000000000000000")
```
```java Java
String requestId = String.valueOf(System.currentTimeMillis());
client.withdraw("GETH",requestId,"toAddr","10000000000000000", null, null, null);
mpcClient.createTransaction("GETH", requestId, "10000000000000000", "fromAddr", "toAddr",
                toAddressDetails, fee, gasPrice, gasLimit, operation, extraParameters, null, null, null);
```
</CodeGroup><br />

Now you may use the `request_id` to query the transaction status. Please note that the transaction will only be confirmed once the on-chain confirmation blocks reach `confirming_threshold`.
...
<CodeGroup>
```python Python
client.get_transactions_by_request_ids(request_ids=request_id)
mpc_client.transactions_by_request_ids(request_id)
```
```java Java
client.getTransactionsByRequestIds(requestId);
mpcClient.transactionsByRequestIds(requestId, null);
```
</CodeGroup><br />
