...
const { MPCClient } = require("cobo-custody");
const { LocalSigner } = require("cobo-custody");
const { DEVELOP, PROD } = require("cobo-custody");

// client for Custodial Wallet
const client = new Client(signer, DEVELOP, true);

// client for MPC Wallet
const mpc_client = new MPCClient(signer, DEVELOP, true);
```
...
```javascript
const { LocalSigner } = require("cobo-custody");
const signer = new LocalSigner("YOUR_API_SECRET");
```
