...
(for non-web3 wallet)input_selection: inputs to be excluded and included in transactions associated with the UTXO model; inputs must be in correspondence with from_address (if specified); it must be a JSON string and has a structure that fits InputSelection
(for web3 wallet)calldata: request data associated with an EVM transaction, type: string;
</ParamField>
