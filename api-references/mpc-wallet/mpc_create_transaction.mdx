...
<ParamField body="max_fee" type="Int" >The maximum fee (unit is wei) refers to the highest miner fee that a user is willing to pay,the parameter is only used when users employ the EIP-1559 protocol in Web3 wallets. </ParamField>
<ParamField body="max_priority_fee" type="Int" >The maximum priority fee (unit is wei) refers to the highest amount a user sets for the priority fee. If the total transaction fee (base fee + max priority fee) exceeds the maximum miner fee, any excess in the fee will be refunded to the user, the parameter is only used when users employ the EIP-1559 protocol in Web3 wallets.</ParamField>
