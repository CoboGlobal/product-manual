...
<ResponseField name="waiting_audit" type="Bool"> When a transaction is in the pending state for deposit transactions and requires auditing, "waiting_audit" is set to true. For withdrawal transactions, it always returns false.</ResponseField>