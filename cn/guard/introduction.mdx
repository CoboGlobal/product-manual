...
多因素认证 (MFA) 是 Cobo Portal 中的一项重要安全措施。在登录时，除密码外，您还需通过 MFA 进行额外身份验证，为账户增加一层保护，防止密码泄露后的未经授权访问。

Cobo Guard 作为一种 MFA 方式，可用于快速验证登录或关键操作（如邀请新用户、修改风险控制）。只需在 Cobo Guard 上点击确认，即可验证用户身份，确保账户安全无虞。

以下截图展示了如何使用 Cobo Guard 验证是你本人发起的操作：
...
如需了解如何使用 Cobo Guard 对交易进行签名，请参阅[移动端签名人](/cn/portal/mpc-wallets/ocw/holder-group-main-group#signer-type)。
