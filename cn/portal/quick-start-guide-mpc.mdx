...
* 对于服务端签名人：
  ...
  * 如果配置了 [TSS Node 回调服务器](https://www.cobo.com/developers/v2/guides/mpc-wallets/server-co-signer/callback-server-overview)，则只有在回调服务器批准后才会开始生成过程。
...
3. 点击**创建钱包**。在对话框中：
   - 输入一个唯一的钱包名称。
   - 如果您需要自动归集多个地址的资金，请启用[自动归集](/cn/portal/mpc-wallets/ocw/auto-sweep-overview)。如果您不确定，可以暂时禁用它，稍后需要时再启用。
4. 点击**创建**。
<img src="/cn/images/get-started/create-wallet.png" className="screenshot_full_screen" alt="创建钱包"/>
...
<img src="/cn/images/risk-controls/create-an-address-list.png" className="screenshot_full_screen"/>
4. 输入地址列表名称，然后点击**确认**。
...
5. 系统将根据在线状态和其他条件自动选择一个签名人来签署交易。请确保至少有一个签名人可以完成签名。
...
1. 参考[发送您的第一个请求](https://www.cobo.com/developers/v2/guides/get-started/get-started-with-waas)获取有关 API Key 注册、身份验证和 SDK 使用的详细说明。
2. 参考[集成 MPC 钱包（机构钱包）](https://www.cobo.com/developers/v2/guides/mpc-wallets/get-started-ocw)获取有关创建钱包和管理代币交易的详细说明。
...
- [备份您的私钥分片](/cn/portal/security-checklist#key-share-backup-and-recovery)
- 除主控组外，[至少创建一个签名组和一个恢复组](/cn/portal/security-checklist#key-share-holder-groups)。
...
## 了解更多
- [观看产品视频](https://www.youtube.com/watch?v=kC-Lp75RBHs)
- [访问官方网站](https://www.cobo.com/products/wallet/mpc)
