...
3. 点击**创建钱包**。在对话框中：
   - 输入一个唯一的钱包名称。
   - 如果您需要自动归集多个地址的资金，请启用[自动代币归集](/cn/portal/mpc-wallets/ocw/auto-sweep-overview)。如果您不确定，可以暂时禁用它，待需要时再启用。
4. 点击**创建**。
...
* [启用自动代币归集](/cn/portal/mpc-wallets/ocw/auto-sweep-overview)：自动将多个地址的资金归集到一个指定地址。
