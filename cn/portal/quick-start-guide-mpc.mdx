...
成功生成私钥后，系统会自动为您的 Vault 创建一个默认钱包，并为 7 条[默认启用的公链](/cn/portal/enable-or-disable-chains#默认启用的公链)（Ethereum、BNB Smart Chain、Bitcoin、Arbitrum、Solana、Base、Polygon）生成对应地址。其中所有 EVM 链共用同一个 `0x` 地址，无需重复添加。
