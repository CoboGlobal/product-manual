...
3. 对于**钱包类型**，选择**MPC 钱包（机构钱包）**。

4. 对于钱包地址，可以从下拉菜单中选择，或通过地址、Vault 名称、钱包名称或标签进行搜索，结果中将列出通过 Cobo Portal 和 WaaS 2.0 API 创建的 MPC 钱包（机构钱包）。

5. 选择转账的代币类型和网络。可选代币类型仅限于团队账户的订阅包中所含的代币。要启用新的代币类型（额外收费），请参阅[从外部钱包充币](https://manuals.cobo.com/cn/portal/mpc-wallets/ocw/transfers#)中的步骤 1 到 6。
...
7. 输入转账金额。注意，对于 DOT、XRP 或 KSM 代币，所选的 MPC 钱包（机构钱包）中持有的余额必须满足最低要求。DOT 代币的最低余额为 1 DOT，XRP 为 10 XRP，KSM 为 0.000333333 KSM。
