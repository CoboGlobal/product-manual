...
1. 对于**钱包类型**，选择**MPC 钱包（机构钱包）**。

2. 对于钱包地址，可以从下拉菜单中选择，或通过地址、Vault 名称、钱包名称或标签进行搜索，结果中将列出通过 Cobo Portal 和 WaaS 2.0 API 创建的 MPC 钱包（机构钱包）。

3. 选择转账的代币类型和网络。可选代币类型仅限于团队账户的订阅包中所含的代币。要启用新的代币类型（额外收费），请参阅[从外部钱包充币](https://manuals.cobo.com/cn/portal/mpc-wallets/ocw/transfers#)中的步骤 1 到 6。
...
4. 在**私钥分片管理**页面，找到您的私钥分片持有者组，并查看每个私钥分片持有者旁边的状态指示器：
   - 🟢 绿点：签名人在线并准备签署交易
     - 对于服务端签名人，表示持有者已启动 TSS Node。
     - 对于移动端签名人，表示持有者已打开 Cobo Guard。
   - 无绿点：签名人离线，无法签署交易。
