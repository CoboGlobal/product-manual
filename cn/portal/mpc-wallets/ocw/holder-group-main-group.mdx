...
<table class="six-columns">
  ...
  <tr>
    ...
    <td>
      服务端签名人是一个服务器节点，用于自动通过 TSS Node 软件包签署交易。其 TSS Node ID 通过 Cobo 提供的 TSS 软件包生成，且必须在设置钱包之前生成。有关进一步说明，请参见[服务端签名人介绍](https://www.cobo.com/developers/v2/guides/mpc-wallets/server-co-signer/tss-node-overview)。<Info>对于高频交易，我们建议选择服务端签名人进行自动签名。</Info>
    </td>
  </tr>
  <tr>
    <td>前提步骤</td>
...
4. 在**生成主控组私钥分片**对话框中，前往**私钥分片持有者 2**，然后完成以下配置：<a id="on-the-create-main-group-dialog"></a>
    ...
    - 要使用 **服务端签名人**，输入**持有方**姓名，填写使用 TSS Node 软件包生成的 **TSS Node ID**，然后点击**确认**。有关进一步说明，请参见[服务端签名人介绍](https://www.cobo.com/developers/v2/guides/mpc-wallets/server-co-signer/tss-node-overview)。
5. 请通知相关的私钥分片持有者进行确认。
    - **注意：**
