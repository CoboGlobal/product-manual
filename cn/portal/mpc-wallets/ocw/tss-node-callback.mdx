...
- 使用 CallbackRequestJsonString 作为 JWT 有效负载，并使用其 RSA 私钥对其进行签名，以创建 JWT。
- 通过 HTML 表单提交 HTTP POST 请求，将 TSS_JWT_MSG 作为密钥，将 JWT 作为值。

Callback 服务器将按以下方式处理 HTTP 请求：

- 使用 TSS_JWT_MSG 作为密钥，JWT 作为值，读取 HTTP POST 请求中的数据。
- 使用 TSS Node 的 RSA 公钥验证已签名的 JWT。
- 读取 JWT 中的有效负载，并使用 CallbackRequest 对其进行反序列化。
- 反序列化元字段，以获取请求中的信息。

### **风险控制**

对于每个 CallbackRequest，必须根据团队的安全要求进行风险控制评估。这一步骤对于确保请求的有效性至关重要。
...
- 生成 CallbackResponse。 
- 序列化 CallbackResponse，生成 CallbackResponseJsonString。 
- 使用 CallbackResponseJsonString 作为 JWT 有效负载，并使用其 RSA 私钥对其进行签名，以创建 JWT。 
...
| request_type   | int    | callback 请求的类型枚举如下： <br></br>TypeKeyGen: 1 <br></br>TypeKeySign: 2 <br></br>TypeKeyReshare: 3 |
...
| **参数**  | **类型** | **描述**                                              |
| -------------- | -------- | ------------------------------------------------------------ |
| cobo_id        | string   | 密钥生成（KeyGen）请求的唯一 ID。            |
...
| api_request_id     | string                   | 提币请求的请求 ID。如果请求通过 API 初始化，其值应与 request_id 相同。如果请求通过 Cobo Portal 初始化，Cobo 将自动生成一个请求 ID。|
...
| amount             | string                   | （可选）该交易金额可以包含小数位（例如，将 1 BTC 除到小数点后八位，100,000,000 代表 1 BTC）。|
| to_address         | string                   | 收币地址。                                             |
| to_address_details | json                     | （可选）适用于 UTXO 模型的收币地址列表。该值必须符合 list[ToAddressDetail] 的 JSON 结构。|
| fee                | int                      | 对于 BTC 交易，该字段返回每个字节的交易费用，单位为 satoshi。|
| gas_price          | int                      | （可选）对于 ETH 交易，该字段返回 gas 费用，单位为 gwei。|
| gas_limit          | int                      | （可选）对于 ETH 交易，该字段返回 gas 限额。|
| extra_parameters   | json                     | 交易的附加参数。请参阅 create_transaction 的说明。|
| replace_cobo_id    | string                   | （可选）被指定为“费用替换（RBF）”的 Cobo 交易的 ID。|
| api_key            | string                   | （可选）通过 API 初始化的提币请求的 API_KEY。|
| spender            | string                   | （可选）提币人的电子邮件信息。适用于通过 Cobo Portal 发起的提币。|
| raw_tx             | list[RawTx]              | 原始交易数据。                                        |
| note               | string                   | 交易备注。                                         |
...
| **参数**     | **类型**    | **描述**                                              |
| ----------------- | ----------- | ------------------------------------------------------------ |
| inputs_to_spend   | list[Input] | 待支出的未支出交易输出 (UTXO)。适用于 UTXO 类型的区块链，如比特币。|
| inputs_to_exclude | list[Input] | 要从支出中排除的未支出交易输出 (UTXO)。适用于 UTXO 类型的区块链，如比特币。|
...
| **参数**   | **类型** | **描述**                                              |
| --------------- | -------- | ------------------------------------------------------------ |
| raw_tx          | string   | 十六进制编码的交易信息。                         |
| derivation_path | string   | 待签名的派生路径。例如，"m/44/60/3/0/1"。|

当**request_type**等于**TypeKeyReshare**时，**request_detail**的结构如下：

| **参数** | **类型** | **描述**                                              |
| ------------- | -------- | ------------------------------------------------------------ |
| old_group_id  | string   | 以前的组 ID。                                           |
| root_pub_key  | string   | 根扩展公钥。                                    |
| curve         | int      | 使用的签名算法：<br></br>0: SECP256K1<br></br>2: ED25519 |
| used_node_ids | []string | 用于私钥恢复的上一个组的选定节点 ID。|
| old_threshold | int      | 以前的签名阈值。                                |
| new_threshold | int      | 新签名阈值。                                     |
| new_node_ids  | []string | 与 MPC 钱包下所有私钥分片生成相关的新 TSS Node ID。|
