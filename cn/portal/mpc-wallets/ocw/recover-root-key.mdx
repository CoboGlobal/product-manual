...
**注意**：如果 Cobo Portal 在极端情况下停止服务，您可以利用活跃的恢复组重建与 Vault 相对应的根扩展私钥。与该 Vault 内钱包地址相关联的所有私钥都可以从这些根扩展私钥中获取。与还原不同，恢复需要依赖另一个私钥分片组的支持。
...
请按照以下说明重新构建所有私钥分片，并派生出与 Vault 相关联的所有钱包地址的私钥。本说明适用于移动端签名人和 API 签名人。
