---
title: "恢复根扩展私钥"
sidebarTitle: "恢复根扩展私钥"
---

**注意**：万一 Cobo Portal 停止服务，您可以利用活跃的恢复组重建与 vault 相对应的根扩展私钥。与该 vault 内钱包地址相链接的所有私钥都可以从这些根扩展私钥中获取。与还原不同，恢复需要依赖另一个私钥分片组的支持。
...
4.（仅限第三方恢复）与受托第三方分享与您打算导出的特定私钥分片链接的根扩展公钥。受托第三方将利用此信息验证其私钥分片信息。
...
## 步骤 2：重新构建所有私钥分片并获取所有钱包地址的私钥

请按照以下说明重新构建所有私钥分片，并获取与 vault 相关联的所有钱包地址的私钥。本说明适用于移动端签名人和 API 签名人。
...
- 使用以下任一方法保存与每个 vault 相关联的钱包地址：
    - 在私钥生成过程中，通过 generate_addresses 操作检索 `AddressDetail`。返回的结果中每个地址对应的 HD 路径。
    - 定期通过 list_addresses 操作检索 `AddressDetail。返回的结果中每个地址对应的 HD 路径。
...
- cobo-mpc-recovery-tool 将使用第一个私钥分片文件中的“participants”信息生成并验证根扩展公钥。
- 需要手动输入与第一个私钥分片文件相关的密码解密。然后，cobo-mpc-recovery-tool 会生成并验证相应的公钥。
- cobo-mpc-recovery-tool 将验证与第二个私钥分片文件相关联的密钥/值。
- cobo-mpc-recovery-tool 将验证第一个私钥分片文件中的密钥/值与第二个私钥分片文件中的密钥/值是否一致。
- cobo-mpc-recovery-tool 将使用第二个私钥分片文件中的“participants”信息生成并验证根扩展公钥。
- 需要手动输入与第二个私钥分片文件相关的密码进行解密。然后，cobo-mpc-recovery-tool 会生成并验证相应的公钥。
