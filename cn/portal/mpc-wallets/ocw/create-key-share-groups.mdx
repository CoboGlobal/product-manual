...
5. 请通知相关的私钥分片持有方进行确认。
    - **注意：**
        - 如果使用移动端签名人，Cobo Portal 上会出现一个**确认**页面。请提醒选定的私钥分片持有方在其 Cobo Guard 中确认。在选定的私钥分片持有方的 Cobo Guard 应用中，点击 **Security Verification** (安全验证) 消息以打开 **Security Verification** (安全验证)页面。审核信息后，点击 **Yes, it is me** (是的，这是我)。一个 **Become Key Holder** (成为私钥分片持有方)的消息接着很快将发送到私钥分片持有方的 Cobo Guard，点击该消息以打开 **Become Key Holder** (成为私钥分片持有方)页面，审核信息后，点击**批准**。选定的私钥分片持有方现在应该在他们的 Cobo Guard 上点击**生成私钥分片**横幅，进入**我的账户**页面，然后在**私钥分片**下点击**等待私钥分片生成**以进入**私钥分片**页面。在**私钥分片**页面上，点击**开始**进入**生成私钥分片**页面。私钥分片生成过程将开始。当 Cobo Guard 上显示**私钥分片已生成**消息时，点击**确定**。
        - 如果使用 API 签名人，请确保在 24 小时内让 TSS 节点上线。TSS 节点上线后，确认流程即告完成。

6. 私钥分片生成过程于第 5 步完成后，**私钥分片组状态**将显示为**活跃**。
