...
5. 请通知相关的私钥分片持有方进行确认。
    - **注意：**
        - 如果使用移动端签名人，会出现一个**确认**页面，提醒选定的私钥分片持有方在其 Cobo Guard 中确认。在选定的私钥分片持有方的 Cobo Guard 应用中，点击 **Security Verification** (安全验证) 消息以打开 **Security Verification** (安全验证)页面。审核信息后，点击 **Yes, it is me** (是的，这是我)。一个 **Become Key Holder** (成为私钥分片持有方)的消息将发送到私钥分片持有方的 Cobo Guard，点击该消息以打开 **Become Key Holder** (成为私钥分片持有方)页面，审核信息后，点击**批准**。
        - 如果使用 API 签名人，请确保在 24 小时内让 TSS 节点上线。TSS 节点上线后，确认流程即告完成。

6. 私钥分片持有方确认后，将开始生成私钥。
<Info>您可能需要刷新页面以查看结果。</Info>
