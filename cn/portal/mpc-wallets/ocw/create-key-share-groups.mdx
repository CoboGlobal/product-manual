...
|------------------------|------------|-------------|
| 主要用途       | 移动端签名人是一名团队成员，负责安全签署通过 Cobo Portal 发起的交易。其 TSS 节点 ID 由 Cobo Guard App 生成，并将自动填入。| API 签名人是一个服务器节点，用于自动签署通过 API 发起的交易。其 TSS 节点 ID 通过 Cobo 提供的 TSS 节点软件包生成，且必须在设置钱包之前生成。有关进一步说明，请参见 [TSS 节点技术设置](/cn/portal/mpc-wallets/ocw/network-environments)。|
| 前提步骤 | 您选择的私钥分片持有方需要完成以下操作：<ol><li>[下载 Cobo Guard](/cn/guard/download)。</li><li>[将 Cobo Guard 设置为 Cobo Accounts 多重身份验证 (MFA) 方法之一](/cn/accounts/set-up-mfa#set-up-cobo-guard)。</li><li>被[添加为您的团队成员](/cn/portal/organization/manage-members)。</li></ol>| 您选择的私钥分片持有方需要向您提供他们生成的 TSS 节点 ID。有关进一步说明，请参见 [TSS 节点技术设置](/cn/portal/mpc-wallets/ocw/network-environments)。|
...
4. 在**生成主控组私钥分片**对话框中，前往**私钥分片持有方 2**，然后完成以下配置：<a id="on-the-create-main-group-dialog"></a>
    ...
    - 要使用 **API 签名人**，输入**持有方**姓名，填写使用 TSS 节点软件包生成的 **TSS 节点 ID**，然后点击**确认**。有关进一步说明，请参见 [TSS 节点技术设置](/cn/portal/mpc-wallets/ocw/network-environments)。
...
1. 在**创建恢复组**对话框中：
    - 对于**私钥分片持有方 1**，输入**持有方**名称，并要求受托第三方提供 **TSS 节点 ID**。
