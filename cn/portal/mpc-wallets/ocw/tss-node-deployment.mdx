...
```
cobo-tss-node-generic-<VERSION>.tgz (TSS Node 软件包)

packagecobo-tss-node-generic-<VERSION>.tgz.sha256 (TSS Node 软件包哈希文件）
```
...
为此，请复制一个 cobo-tss-node-config.yaml.template 文件，将其重命名为 cobo-tss-node-config.yaml。将新文件粘贴到 configs 目录中。有关说明，请参阅 [TSS Node 配置方法](/cn/portal/mpc-wallets/ocw/appendix#tss-node-configuration-methods)。配置文件修改完毕后，切记重新启动 TSS Node 软件包。
...
- 将自动生成并打印 callbeck 私钥。有关更多信息，请参阅 [TSS Node callback](/cn/portal/mpc-wallets/ocw/tss-node-callback)。
...
9. 确保 Cobo Portal 上显示的根扩展公钥与 TSS Node 日志中显示的根扩展公钥相符。此私钥可用于使用 BIP 32 生成此 MPC 机构钱包下的所有钱包地址。

### **私钥分片管理** 

成功生成的私钥分片将被加密，并存储在 TSS Node 软件包的本地数据库文件中。默认路径为 db/secrets.db。强烈建议在初始化 TSS Node 时备份数据库文件和用于加密的密码。备份文件应存储在不同的设备上，以提高安全性。有关更多信息，请参阅[备份私钥分片组](/cn/portal/mpc-wallets/ocw/back-up-key-share-groups)。