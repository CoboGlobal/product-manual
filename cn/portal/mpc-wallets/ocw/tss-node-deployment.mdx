...
Note: If you are using an Apple MacBook, please execute the following command to check the SHA256 (256-bit) checksums.
...
- The callback key will be automatically generated and printed. For more information, please refer to [TSS Node callback](/cn/portal/mpc-wallets/ocw/tss-node-callback).
...
You can press Ctrl+C to exit, and the TSS Node will continue to run in the background. For more information on checking the running status of the TSS Node, please refer to the **appendix**.
...
*Note: Only the client with Vault Admin rights is authorized to add the TSS Node ID to Cobo Portal.*
