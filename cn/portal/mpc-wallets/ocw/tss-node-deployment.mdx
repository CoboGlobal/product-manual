...
- **通用版本**：兼容所有服务器，包括云服务商的服务器、用户自建的服务器、通用服务器以及 Apple MacBooks。虽然此版本也可以在支持 SGX 的服务器上运行，但 SGX 相关功能将不会启用。此版本 TSS Node 软件包可在[这里](https://cobo-tss-node.s3.amazonaws.com/generic/latest/cobo-tss-node-generic.tgz)下载。
