...
- SGX 安全增强版：适用于 SGX 服务器，软件包名为 cobo-tss-node-linux-sgx-\<VERSION\>.tgz。
- 通用版本：专为通用服务器或 Apple MacBook 设计，软件包名为 cobo-tss-node-generic-\<VERSION\>.tgz。

要获取适合您的部署方法的 TSS Node 软件包，请联系 Cobo 客服，联系方式：[support@cobo.com]。下面的示例使用了适用于通用服务器的 TSS Node 软件包。

```
cobo-tss-node-generic-<VERSION>.tgz (TSS Node 软件包)

packagecobo-tss-node-generic-<VERSION>.tgz.sha256 (哈希文件）
```
...
所有三种运行环境（即 SGX 服务器、通用服务器和 Apple MacBook）的 TSS Node 命令行是一致的。但命令行的执行可能略有不同。对于 SGX 服务器和通用服务器，部署和运行 TSS Node 需要 sudo 访问权限。换言之，要确保插入 sudo 命令。
...
请参看[创建主控组](/cn/portal/mpc-wallets/ocw/create-key-share-groups#create-main-group)了解将 TSS Node ID 添加到 Cobo Portal 的具体步骤。

确保 Cobo Portal 上显示的根扩展公钥与 TSS Node 日志中显示的根扩展公钥相符。你可以使用 BIP 32 基于此公钥生成该 MPC 机构钱包下的所有钱包地址。
