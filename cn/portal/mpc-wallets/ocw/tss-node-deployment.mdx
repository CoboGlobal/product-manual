...
- **SGX 安全增强版**：专为配备 SGX 功能的服务器设计。此版本 TSS Node 软件包可在[这里](https://cobo-tss-node.s3.amazonaws.com/linux-sgx/latest/cobo-tss-node-linux-sgx.tgz)下载。
- **通用版本**：兼容所有服务器，包括云服务商的服务器、用户自建的服务器、现成服务器以及 Apple MacBooks。虽然此版本也可以在支持 SGX 的服务器上运行，但 SGX 相关功能将不会启用。此版本 TSS Node 软件包可在[这里](https://cobo-tss-node.s3.amazonaws.com/generic/latest/cobo-tss-node-generic.tgz)下载。
...
所有运行环境的 TSS Node 命令行是一致的。但命令行的执行可能略有不同。对于 SGX 服务器和现成服务器，部署和运行 TSS Node 需要 sudo 访问权限。换言之，要确保插入 sudo 命令。
