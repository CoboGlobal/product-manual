...
注意，TSS 节点的配置不是强制性的。下面列出的所有项目均为可选项。
...
您可以初始化 TSS 节点并创建数据库文件。如果数据库文件已经存在，则执行将失败。有关更多信息，请参阅 [TSS 节点初始化](/en/portal/mpc-wallets/ocw/tss-node-deployment#tss-node-initialization)。
...
您可以查看与 TSS 节点相关的信息，包括 TSS 节点 ID 和 Callback 服务器的 RSA 公钥。 
...
| Root Extended Public Key | xpub661MyMwAq……Mcet8                    | 根扩展公钥对应当前组下的私钥分片。|
| ChainCode                | 0x098a……9dd9                            | ChainCode 对应根扩展公钥，为十六进制格式。|
| Share ID                 | 121637……483926                          | MPC 私钥分片的分片 ID，为十进制格式。             |
| Share Public Key         | 0x0231adf……d2773                        | 公钥与私钥分片相对应：默认值：压缩公钥，为十六进制格式。|
| Type                     | 1                                       | 默认值为 1。                                          |
| Curve                    | secp256k1                               | 使用的签名算法。                      |
| Threshold                | 2                                       | 每笔交易签名所需的最小私钥分片数量。|
| Local Node ID            | coboA1B2C3D4                            | 本地存储的 TSS 节点的节点 ID。                      |
| Other Node IDs           | [coboA1B2C3D4, coboA2B3C4D5]            | 对方管理的 TSS 节点的节点 ID。          |
| Participants：            | 3                                       | 私钥分片总数、节点 ID 和私钥分片 ID。|
...
cobo-tss-node 将开始运行。有关更多信息，请参阅 [TSS 节点启动](/en/portal/mpc-wallets/ocw/tss-node-deployment#tss-node-startup)。
...
TSS 节点具有集成的风险控制机制，可在启动 Callback 之前评估私钥生成（KeyGen）、私钥签名（KeySign）或私钥恢复（KeyReshare）请求。系统对这些请求应用预定义的风险控制策略，只批准符合指定策略的请求。不符合标准的请求将被拒绝。 

您可以根据您的业务要求，定制 TSS 节点风险控制策略的配置，使二者保持最佳的一致性。为此，可通过 configs/cobo-tss-node-config.yaml 文件进行定制配置。要定义特定的配置参数，请导航至 embedded_risk_control_rules，并参考以下示例：
...
key_gen: （KeyGen 风险控制策略）

reject_all: false （true：将拒绝所有 KeyGen 请求；false：将按照以下策略检查所有 KeyGen 请求）

allow_list: （KeyGen 的风险控制策略；仅当符合策略时，才会批准请求）
...
key_sign: （KeySign 风险控制策略）

reject_all: true （true：将拒绝所有 KeySign 请求；false：将批准 KeySign 请求）

key_reshare: （KeyReShare风险控制策略）

reject_all: false （true：将拒绝所有 KeyReShare 请求；false：将根据以下策略检查 KeyReShare 请求）

allow_list: （KeyReShare 的风险控制策略；仅当符合策略时，才会批准请求）
...
注意，需要将占位符 \<EXTENDED_ROOT_PUBKEY\>、\<Cobo TSS 节点 ID\>、\<第三方 TSS 节点 ID\> 和 \<客户 TSS 节点 ID\> 替换为实际值。输入这些参数后，删除“\<\>”符号。
