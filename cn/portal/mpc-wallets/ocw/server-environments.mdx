...
Intel® Software Guard Extensions (SGX) 提供基于硬件的内存加密，可隔离内存中的特定应用程序代码和数据。SGX 允许执行客户端代码，分配私有内存区域，称为 enclave。enclave 是专门设计的受保护部分，可防止更高特权级别的进程访问这些内存区域。它可以提供粒度级别的控制和保护，抵御许多已知的、活跃的威胁。要了解有关 Intel® SGX 的更多信息，请[点击此处](https://www.intel.com/content/www/us/en/developer/tools/software-guard-extensions/overview.html)。 

### 支持 SGX 的服务器类型

#### Azure 机密计算（虚拟机）

配置支持 SGX 的服务器需要满足以下设置：
...
要详细了解如何使用 Azure 门户部署支持 SGX 的服务器，请[点击此处](https://learn.microsoft.com/en-us/azure/confidential-computing/quick-create-portal)。
...
要详细了解如何使用阿里云弹性计算服务部署支持 SGX 的服务器，请[点击此处](https://www.alibabacloud.com/help/en/ecs/user-guide/build-an-sgx-encrypted-computing-environment)。
...
- 选择**是，同时查找 Intel® SPS 和 Intel® ME**
- 查看产品规格并配置以下设置
...
默认情况下，已经安装 SGX 驱动程序。在 TSS Node 初始化过程中，系统将提示您同意自动安装 SGX 驱动程序 (Intel DCAP 1.41)。要验证是否安装，请执行以下命令。
...
默认情况下，5.1 及以上版本的 SGX 驱动程序已集成到 Linux 内核中。建议使用 Linux 5.1 或更高版本。
...
现成服务器是指满足 TSS Node 最低配置前提的任何服务器，如弹性计算服务器或由客户端托管的物理服务器。虽然现成服务器可以托管 TSS Node，但它缺乏 SGX 服务器特有的安全功能。
