...
Intel® Software Guard Extensions (SGX) 提供基于硬件的内存加密，可隔离内存中的特定应用程序代码和数据。SGX 允许执行客户端代码，分配私有内存区域，称为 Enclave。Enclave 是专门设计的受保护部分，可防止更高权限级别的进程访问这些内存区域。它可以提供细颗粒度的控制和保护，抵御许多已知的、活跃的威胁。要了解有关 Intel® SGX 的更多信息，请[点击此处](https://www.intel.com/content/www/us/en/developer/tools/software-guard-extensions/overview.html)。 
