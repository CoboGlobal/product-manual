...
您可以轻松设置和编辑交易风控，通过设置审批动作来自动处理每笔交易。
<Note>交易风控由 Cobo Portal 的后端系统管理，适用于所有钱包类型。</Note>

交易风控是您团队的重要安全保障之一，建议您严格配置交易风控策略，特别是在大额提币场景中。应特别注意交易风控中的代币转账策略，以避免高风险情况，例如：
- 没有设置风控规则
- 存在未被交易风控覆盖的交易
- 所有交易自动通过
- 同一角色既可提币又可审批
