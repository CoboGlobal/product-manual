...
<Note>角色权限和 Wallet Scope 是两个独立控制项。您选择的角色会在资源和操作维度授予 API 操作权限，而 Wallet Scope 会限制这些权限可作用的钱包范围。对于带钱包目标的 Custody 和 WaaS 操作，角色权限和 Wallet Scope 都必须允许该操作。完整权限矩阵请参阅[角色与权限](/cn/portal/risk-controls/roles-and-permissions)。</Note>
...
8. 从对应的下拉列表中选择该 API Key 的用户角色以及可访问的钱包范围。
    ...
    使用 **Shared scope** 可将同一个钱包范围应用于所有已选角色。点击**每个角色**可使用 **Per-role scope**，为每个已选角色分别设置钱包范围。
...
9. 选择此 API Key 使用的 Callback Endpoint，或注册一个新的 Callback Endpoint。所选的 Callback Endpoint 将接收由该 API Key 触发的消息。有关更多信息，请参阅[注册 Callback Endpoint](/cn/portal/developer-console/callbacks-register)。
    ...
    <Warning>如果此 API Key 未配置 Callback Endpoint，基于 Callback 的提币二次确认将被跳过，提币请求会默认批准。</Warning>
10. 选择 Key 类型。
