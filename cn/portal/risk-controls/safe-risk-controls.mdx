...
7. 在**如果**下，点击 **+ 添加条件** 并选择 **Safe&#123;Wallet&#125; 消息签名**。为您的签名人地址（在**适用钱包**中选择的 Cobo Portal 地址）提供对应的 Safe&#123;Wallet&#125; 地址。当这些 Safe&#123;Wallet&#125; 地址请求签名时，规则将被触发。
   ...
   开启**自定义条件**以增强安全控制。
      - **自定义委托调用**：仅当委托调用满足指定条件时，规则才会被触发。
      - **自定义合约调用**：仅当合约调用满足指定条件时，规则才会被触发。

   您可以选择多个自定义条件（例如同时选择**自定义委托调用**和**自定义合约调用**），它们之间使用**或**的关系，满足任一条件即触发规则。

   对于每个自定义条件，您可以选择多个合约地址，并选择以下三个粒度之一：
      - **合约级**：您可以指定调用的智能合约地址。当**目标合约**与指定地址之一匹配时，规则将被触发。
      - **方法级**：您可以指定调用的智能合约地址和方法。仅当**目标合约**和**方法**之一都匹配时，规则才会被触发。
      - **参数级**：您可以指定**目标合约**、**方法**和**参数**。仅当**目标合约**的智能合约地址、**方法**和**参数**都匹配时，规则才会被触发。
...
- 对于 multiSend 交易，**所有操作**部分显示打包的操作，每个操作以`合约名称：方法名称`的形式显示。
   ...
   <img src="/cn/images/risk-controls/multisend.png" className="screenshot_full_screen"/>

   点击展开操作以查看详情。

   <img src="/cn/images/risk-controls/multisend-expand.png" className="screenshot_full_screen"/>
