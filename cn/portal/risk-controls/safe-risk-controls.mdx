...
为确保全面保护，创建以下三个兜底规则并将其置于步骤2中创建的 Safe&#123;Wallet&#125; 消息签名规则之下。这些规则将作为安全网，自动拒绝任何未被主规则覆盖的请求。

1. **消息签名兜底规则**：

在**交易风控规则**页面，点击**创建交易风控规则** > **消息签名**。按如下配置规则：
   - **适用链**：选择**任何**
...
在**交易风控规则**页面，点击**创建交易风控规则** > **合约交互**。按如下配置规则：
   - **适用钱包**：选择包含您的签名人地址的钱包
   - **适用链**：选择**任何**
   - 跳过设置条件以覆盖所有合约交互
   - **执行动作**：选择**自动拒绝**
...
在**交易风控规则**页面，点击**创建交易风控规则** > **代币转账**。按如下配置规则：
   - **适用钱包**：选择包含您的签名人地址的钱包
   - 跳过设置条件以覆盖所有代币转账
   - **执行动作**：选择**自动拒绝**
...
- 使用红色标签标记高风险因素，显示风险详情，例如：
   ...
   - 对于 **To** 字段：
      | 字段 | To 地址 | 交易类型 | 风险标签 | 说明 |
      |------|---------|----------|-----------|------|
      | To | Safe&#123;Wallet&#125; 自身 | 合约交互 | **Wallet Config Change** | 钱包配置变更 |
      | To | 非 Safe&#123;Wallet&#125; 自身 | 转账 | **Never Transacted Before** | 首次交互的接收地址 |
      | To | 非 Safe&#123;Wallet&#125; 自身 | 合约交互 | **Never Interacted Before** | 首次交互的合约地址 |
      | To | 非 Safe&#123;Wallet&#125; 自身 | 合约交互 | **New Contract** | 合约部署时间不足30天（720小时） |
   - 对于 **Operation** 字段：
