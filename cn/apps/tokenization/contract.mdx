...
CoboERC20 是一个基于 OpenZeppelin ERC20 标准构建的可升级合约，并增加了权限控制、代币流通限制和完整的生命周期管理等功能。该合约已通过第三方安全审计，确保其稳健性。
...
权限只能在创建代币时分配，代币创建后无法再分配或删除权限（仅权限持有者可以将自己的权限转移给其他地址）；Permanent Delegate 权限一旦设置无法修改。每个权限仅对应一个钱包地址。
- Permanent Delegate
- Mint Authority
- Freeze Authority
- Update Authority
...
Permanent Delegate 是 Token-2022 的一个强大功能，允许指定的地址在任何情况下对代币执行特权操作，包括强制转账和销毁。

<Note>在 Tokenization App 页面，在 Solana 链代币下，点击**更多操作**进入合约操作界面。</Note>

- **Force Burn via Permanent Delegate（通过永久委托强制销毁）**: 永久委托者可以强制销毁任何账户中的代币，无需持有者同意。这在法律合规或紧急情况下特别有用。

- **Force Transfer via Permanent Delegate（通过永久委托强制转账）**: 永久委托者可以强制将代币从任何账户转移到指定地址，绕过正常的授权机制。用于资产冻结、法院命令执行等场景。

### 元数据管理

**Update Metadata URI（更新元数据URI）**: 更新代币的元数据信息，包括名称、符号、描述、图标等。元数据以 JSON 格式存储，支持动态更新代币信息。
