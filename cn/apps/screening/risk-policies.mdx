...
<table class="table six-columns">
  ...
  <tbody>
    <tr>
      <td>批准</td>
      <td>• 入金：释放资金<br/>• 出金：允许继续</td>
    </tr>
    <tr>
      <td>批准并发出警报</td>
      <td>• 入金：释放资金并发送风险交易通知<br/>• 出金：允许继续并发送风险交易通知</td>
    </tr>
    <tr>
      <td>拒绝</td>
      <td>• 入金：<br/>&nbsp;&nbsp;• 标记交易并且冻结资金 <br/>&nbsp;&nbsp;&nbsp;&nbsp;• *注意：如果您使用 [MPC 钱包的自动代币归集功能](/cn/portal/auto-sweep-overview)，冻结的资金将不会被包含在归集操作中。*<br/>• 出金：阻止交易<br/>• 始终发送风险交易通知</td>
    </tr>
    <tr>
      <td>人工审核</td>
...
例如：
```
规则 1：入金 + 中等风险 + ≥ 500 + < 1000 美元（有效）
规则 2：入金 + 中等风险 + ≥ 400 + < 7000 美元（无效 - 金额范围与规则 1 重复）
规则 2：入金 + 中等风险 + ≥ 600 + < 700 美元（无效 - 金额范围与规则 1 重复）
规则 2：入金 + 中等风险 + ≥ 1000 + < 7000 美元（有效 - 金额范围不重复）
规则 3：出金 + 中等风险 + ≥ 1000 + < 7000 美元（有效 - 不同方向）
```
